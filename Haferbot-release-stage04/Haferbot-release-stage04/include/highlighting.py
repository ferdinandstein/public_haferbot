...
def debug_print(message: str, zustand):
    if zustand.debug_enabled:
        if os.name == "posix":
            # Auf Linux/macOS reicht ein einfaches print
            print(f"\x1b[1m\x1b[38;5;73m{message}", file=sys.stderr)
        else:
            print(f"\x1b[1;38;5;73m{message}\x1b[0m", file=sys.stderr)


def bunt(text: str) -> str:
    ergebnis = []
    farbe = random.randint(30, 37)  # Nur Standard-Farben: 30-37
    ergebnis.append(f"\x1b[1;{farbe}m")

    for zeichen in text:
        if not zeichen.isspace():
            ergebnis.append(zeichen)
        else:
            # Volles 256-Farben-Spektrum nutzen
            farbe = random.randint(30, 37)  # Nur Standard-Farben: 30-37
            ergebnis.append(f"\x1b[{farbe}m{zeichen}")

    ergebnis.append("\x1b[0m\n")
    return "".join(ergebnis)


def get_terminal_name():
    # Speziell für VS Code, iTerm2, Apple Terminal
    program = os.environ.get("TERM_PROGRAM")
    if program:
        return program

    # Standard auf fast allen Linux/Unix-Systemen (z.B. xterm, screen, tmux)
    term = os.environ.get("TERM")
    if term:
        return term

    # Falls wir auf Windows in der CMD oder PowerShell sind
    if os.name == "nt":
        return "Windows Console (CMD/PowerShell)"
...
def highlight_candidates_by_antenna(zustand: Zustand):
    highlight_items = []
    colors = [
        "#fffb00",
        "#ffbb00",
    ...
    if zustand.debug_enabled == True:
        i = 0
        for x in zustand.candidates_by_antenna:
            if len(x) > 0:
                for tile in x:
...
def highlight_candidates(zustand: Zustand):
    ...
    if zustand.debug_enabled == True:
        for tile in list(zustand.candidates[0]):
            converted_item = [tile[0], tile[1], "#ffcc00"]
            highlight_items.append(converted_item)
        for tile in list(zustand.candidates[1]):
            converted_item = [tile[0], tile[1], "#f57309"]
            highlight_items.append(converted_item)
    return highlight_items
