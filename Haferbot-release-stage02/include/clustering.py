...
def cluster_frontiers(zustand: Zustand):
    ...
    # Frontier als set
    frontier_set = set((p.x, p.y) for p in zustand.frontiers)
    ...
    for start in zustand.frontiers:
        ...
        clusters.append([Position([x, y]) for x, y in cluster])
    ...
    if zustand.tick > 50:
        ...
        used_mid = zustand.mid
    else:
        used_mid = get_field_nearest_mid(zustand)

    mids = []
    for cluster in clusters_with_mid:
        mids.append(cluster[1])

    mid_dists = bfs_distances_from(used_mid, zustand, mids)
    ...
    for info in zustand.dictionary_of_all_tiles.values():
        ...
        def get_value_for_age(age):
            value = (-1 / (best_age**2)) * ((age - best_age) ** 2) + 1
            return value
    ...
    def sort_by_path_length_and_points(cluster_with_mid_of_cluster):
        ...
        cluster_mid = cluster_with_mid_of_cluster[1]
        ...
        dist_to_mid = mid_dists[cluster_mid]
        if dist_to_mid == 0:
            dist_to_mid = 1
...
def get_mid_for_each_cluster(zustand: Zustand, clusters):
    ...
    if zustand.with_opponent == True:
        for cluster in clusters:
            #    to_sort = cluster.copy()
            #    to_sort.sort(key=lambda frontier: get_distance_to_nearest_wall(frontier.x, frontier.y, zustand, max_dist=3))
            #    clusters_with_mid.append((cluster, to_sort[0]))
            ...
            avg_position = Position([int(avg_x), int(avg_y)])
            best_delta = 1000
            best_pos = cluster[0]
            for frontier in cluster:
                if frontier == avg_position:
                    ...
                    dx = abs(frontier.x - avg_position.x)
                    dy = abs(frontier.y - avg_position.y)
                    if dx + dy < best_delta:
                        best_delta = dx + dy
                        best_pos = frontier
